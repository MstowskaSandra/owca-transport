...
import { HashLink as Link } from "react-router-hash-link"; // POPRAWKA IMPORTU
...
const ServiceCard = ({ icon: Icon, title, text }) => {
  ...
  return (
    <div className="bg-white border-4 border-[#C2B7B2] rounded-[36px] px-4 py-6 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl shadow-md w-full max-w-85 mx-auto min-h-60 ">
      ...
      <div className="w-full flex justify-end pt-4">
        {/* HashLink automatycznie obsłuży przewinięcie po zmianie podstrony */}
        <Link
          smooth // POPRAWKA: Dodajemy atrybut smooth dla płynnego efektu przewijania
          to={`/uslugi#${slug}`}
          className="font-mont font-bold rounded-full bg-[#EAB33B] hover:bg-[#D46B43] px-6 py-2.5 text-[10px] tracking-widest text-[#353436] shadow-md transition-all duration-200 active:scale-95 uppercase"
        >
          Zobacz
